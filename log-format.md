...
| `component` | `string` | Name of the component that fired (executed its internal transition). |
| `output` | `string \| null` | External output coupling (EOC) value as an interval string, or `null` if the component's output was routed only internally (IC) and not to the coupled model boundary. |
...
Some simulation steps (e.g., initial routing of outputs after a branch fork) may not produce log entries. Not every branch ID will appear in the log — gaps are normal.
...
```jsonl
{"branch": "0.0", "component": "G1", "output": null, "parent_branch": "0", "step": 0, "time": "[0.997, 1.005]"}
{"branch": "0.1", "component": "G2", "output": null, "parent_branch": "0", "step": 0, "time": "[0.997, 1.005]"}
{"branch": "0.2", "component": "G3", "output": null, "parent_branch": "0", "step": 0, "time": "[0.997, 1.005]"}
{"branch": "0.3", "component": "G4", "output": null, "parent_branch": "0", "step": 0, "time": "[0.997, 1.005]"}
```

**Step 0**: All four generators are simultaneously imminent at time `[0.997, 1.005]`. The simulator creates 4 branches from the root `"0"`, one for each generator firing first. In branch `"0.0"`, G1 fires first; in `"0.1"`, G2 fires first; and so on. Output is `null` because generator output is routed internally to the processor (IC), not to the coupled model boundary.

```jsonl
{"branch": "0.0.0.0", "component": "P", "output": "[1, 1]", "parent_branch": "0.0.0", "step": 4, "time": "[1.223, 1.255]"}
```
