...
- **RootCoordinator**: Algorithm 4 from VWD21. BFS simulation loop. Dequeues branches, computes actions, executes (in-place for single action, deepcopy for multiple). Passive branches (t_next=None) are discarded. Safety limits: `max_steps` (graceful stop), `max_branches` (raises `SimulationLimitError`). Returns `list[LogEntry]`. `LogEntry` fields: `branch`, `component`, `kind` (`"atomic"` | `"coupled"` | `"skip"`), `output`, `parent_branch`, `step`, `time`. Skip branches (no component fires) are always logged so `parent_branch` references are never dangling.
